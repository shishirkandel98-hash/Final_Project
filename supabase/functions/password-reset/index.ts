...
async function sendPasswordResetEmail(to: string, newPassword: string): Promise<void> {
  // Hardcoded Gmail credentials as used in other functions
  const gmailUser = "kpoliking001@gmail.com";
  const gmailAppPassword = "nrcd lxrl omvc rpfy";
...
Finance Manager Team
  ...
  await client.send({
    from: `"Finance Manager" <${gmailUser}>`,
    to: to,
    subject: `Finance Manager - Password Reset`,
